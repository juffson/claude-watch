...
「接 USB 时保持常亮」（默认开）：检测到 USB 供电就忽略两个超时，拔掉后才启用——所以可以放心把超时设短。电池续航建议：用电池时设「熄屏 1 分钟 + 深睡 30 分钟」；接 USB 常亮时两者设「从不」。板载电池较小，满速常亮大约只能撑几个小时。

**动态调频**：静止 80 MHz；Claude 页有转圈/脉冲动画、翻页动画、语音进行中 160 MHz；任何交互（触摸、按键、翻页、亮屏）后 240 MHz 保持 3 秒；熄屏待机 80 MHz。串口 `info` 显示当前主频，`perf` 会先升到 240 再测。
